...
def evaluate(model, test_loader, device, num_votes=3):
    ...
    with torch.no_grad():
        ...
        for i, (point, cls, label, question, aff_label) in tqdm(enumerate(test_loader), total=len(test_loader), ascii=True):
            ...
            # ########## MODIFICATION START ##########
            # Initialize a tensor to accumulate logits over all votes
            aggregated_logits = torch.zeros_like(label)

            # Aggregate predictions with voting
            for v in range(num_votes):
                seed_torch(v)  # Set the seed for reproducibility
                
                # model() returns a list of logits from different stages
                all_mask_logits = model(question, point)
                
                # Use the output of the final stage for evaluation
                final_mask_logits = all_mask_logits[-1]
                
                # Accumulate the logits
                aggregated_logits += final_mask_logits
                
            # Average the logits over all votes
            averaged_logits = aggregated_logits / num_votes
            
            # Apply sigmoid to get final probabilities
            _3d = torch.sigmoid(averaged_logits)
            # ########## MODIFICATION END ##########
...
if __name__=='__main__':
    ...
    model = get_PointRefer_Iterative_Einsum(emb_dim=512,
                       ...
                       num_affordance =18)
    model_checkpoint = torch.load("/data3/gaoyuming/Task_Oriented_Grasping/LASO-main_PWCA+mutilstage/runs/train/PointRefer/best_model-try_at_10.20_17.44.29.pt", map_location='cuda:1')
